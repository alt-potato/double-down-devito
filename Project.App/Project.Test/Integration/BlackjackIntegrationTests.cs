...
public record BlackjackTestScenario : IXunitSerializable
{
    ...
    public void Deserialize(IXunitSerializationInfo info)
    {
        ...
        NumberOfPlayers = info.GetValue<int>(nameof(NumberOfPlayers));
        Rounds =
            JsonSerializer.Deserialize<List<RoundScenario>>(info.GetValue<string>(nameof(Rounds)))
            ?? [];
    }
    ...
    public void Serialize(IXunitSerializationInfo info)
    {
        ...
        info.AddValue(nameof(NumberOfPlayers), NumberOfPlayers);
        info.AddValue(nameof(Rounds), JsonSerializer.Serialize(Rounds));
    }
...
public class BlackjackTestHarness(WebApplicationFactory<Program> appFactory)
{
    ...
    ///
    /// Creates a room with the specified number of players, and starts the game.
    /// </summary>
    public async Task SetupGameAsync(int numberOfPlayers)
    {
...
public static class GameTestHarnessExtensions
{
    /// <summary>
    /// Creates a player and adds it to the given list of players.
    ///
    /// If <paramref name="existingRoomId"/> is null, a new room is created with the player as host.
    /// Otherwise, the player is added to the specified room.
    ///
    /// The created player is added to the list, along with its associated HTTP client and SSE reader.
    /// </summary>
    /// <param name="players">The list of players to add the created player to.</param>
    /// <param name="appFactory">The application factory to use when creating the player.</param>
    /// <param name="existingRoomId">The ID of the room to add the player to, or null to create a new room.</param>
    /// <param name="maxPlayers">The maximum number of players allowed in the room, if a new room is created.</param>
    /// <returns>The ID of the room that the player was added to.</returns>
    public static async Task<Guid> CreatePlayer(
        this List<(
            Guid id,
            HttpClient client,
            StreamReader reader,
    ...
    {
        bool isHost = existingRoomId is null;

        // create player
        var playerId = Guid.CreateVersion7();
        var client = appFactory.CreateClient();
        client.DefaultRequestHeaders.Add(TestAuthHandler.TestUserHeader, playerId.ToString());
        ...
        await using (var scope = appFactory.Services.CreateAsyncScope())
        {
            ...
            context.Users.Add(
                ...
                {
                    Id = playerId,
                    // assume host if roomId is null, since new room is created
                    Name = isHost ? "HostPlayer" : $"Player{players.Count}",
                    Email = isHost ? "host@example.com" : $"{players.Count}@example.com",
                }
            );
            await context.SaveChangesAsync();
        }

        // if no room given, create new room new player as host
        var roomId =
            existingRoomId
        ...
        // if room already exists (ie. not host), add player to room (before connecting to SSE)
        if (!isHost)
        {
            await client.AddPlayerToRoomAsync(playerId, roomId);
        }

        var (reader, cts) = await client.OpenSseConnection(roomId);
        players.Add((playerId, client, reader, cts));

        return roomId;
    }

    /// <summary>
    /// Creates a new room with the given options and returns the ID of the created room.
    /// </summary>
    /// <param name="client">The HTTP client to use when creating the room.</param>
    /// <param name="options">The <see cref="CreateRoomDTO"/> used to create the room.</param>
    /// <returns>The ID of the created room.</returns>
    public static async Task<Guid> CreateRoomAsync(this HttpClient client, CreateRoomDTO options)
    {
        ...
        createdRoom!.HostId.Should().Be(options.HostId);
        createdRoom!.GameMode.Should().Be(GameModes.Blackjack);
        createdRoom!.IsActive.Should().BeTrue();

        // ASSERT: initial game state should be not_started
        ...
        var roomExistsResponse = await client.GetAsync($"/api/room/{roomId}/exists");
    ...
    /// </summary>
    public static async Task AddPlayerToRoomAsync(
...
{
    public static TheoryData<BlackjackTestScenario> GetBlackjackTestScenarios()
    {
        return
        [
    ...
    [Theory]
    [MemberData(nameof(GetBlackjackTestScenarios))]
    public async Task BlackjackGameScenario_ShouldCompleteSuccessfully(
        BlackjackTestScenario scenario
    )
    {
        ...
        var appFactory = CreateConfiguredWebAppFactory(
            services =>
            {
                ...
                services.AddSingleton<IRoomSSEService>(
                    new RoomSSEService(NullLogger<RoomSSEService>.Instance)
                );
        ...
        var harness = new BlackjackTestHarness(appFactory);
